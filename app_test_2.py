...
# Create two columns: one for the full chart and one for detection results
...
if ticker:
    while True:
        ...
        if not chart_data.empty:
            with col1:
                # Generate chart image and display in one column
                chart_image = generate_chart_image(chart_data)
                st.image(chart_image, caption=f"{ticker} Chart", use_column_width=True)
            
            with col2:
                # Automatically detect objects and display results in the second column
